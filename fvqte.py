...
def measure_overlap(new_params, original_params, mat, n = 3, mode = 1):
    l = 15
    if mode == 1:
        l = 8
    L = Loss_cirq(n)
...
def local_measure_overlap(new_params, original_params, mat, n = 3, mode = 1):
    l = 15
    if mode == 1:
        l = 8
    local_loss_fn = 0
...
def compute_overlap_and_gradient(params, shift, mat):
    len_param = len(params)
    E = 0
    result = []
    E = measure_overlap(params, params + shift, mat)
    for i in range(len_param):
        result.append(measure_overlap(params + shift + ei(i,len_param)*np.pi/2.0, params, mat))
        result.append(measure_overlap(params + shift - ei(i,len_param)*np.pi/2.0, params, mat))
    #print('E = ', E)
    #print('result = ', result)
    g = []
...
def local_compute_overlap_and_gradient(params, shift, mat):
    len_param = len(params)
    E = 0
    result = []
    E = local_measure_overlap(params, params + shift, mat)
    for i in range(len_param):
        result.append(local_measure_overlap(params + shift + ei(i,len_param)*np.pi/2.0, params, mat))
        result.append(local_measure_overlap(params + shift - ei(i,len_param)*np.pi/2.0, params, mat))
    #print('E = ', E)
    #print('result = ', result)
    g = []
...
def compute_overlap_and_gradient_spsa(params,shift,mat,count):
    ...
    results.append(measure_overlap(params + shift + delta, params, mat))
    results.append(measure_overlap(params + shift - delta, params, mat))
...
def local_compute_overlap_and_gradient_spsa(params,shift,mat,count):
    ...
    E = local_measure_overlap(params, params + shift, mat)
    # Now evaluate the circuits with the parameters assigned
    results.append(local_measure_overlap(params + shift + delta, params, mat))
    results.append(local_measure_overlap(params + shift - delta, params, mat))
...
def fvqte(V, g, h, cost, grad, gd_method, time_steps, max_iter, file_name):
    ...
    for i in range(n_steps):
        ...
        while overlap[0] < ths and count < max_iter:         #ths =0.99, max_iter = 50
            ...
            if cost == 'global':
                if grad == 'param_shift':
                    E,g = compute_overlap_and_gradient(params,shift,WW)
                    
                if grad == 'SPSA':
                    E,g = compute_overlap_and_gradient_spsa(params,shift,WW, count) #spsa

            if cost == 'local':
                if grad == 'param_shift':
                    E,g = local_compute_overlap_and_gradient(params,shift,WW)
                    
                if grad == 'SPSA':
                    E,g = local_compute_overlap_and_gradient_spsa(params,shift,WW, count) #spsa
            ...
            if gd_method == 'momentum':
                ...
                print("Momentum")
                m_grad = 0.7*np.asarray(g) + 0.3*old_grad
                shift = shift - m_grad
